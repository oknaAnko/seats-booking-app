...
export const seatsReducer = (state = initialState, action) => {
  switch (action.type) {
    ...
    case types.TOGGLE_CHOSEN_SEATS:
      return {
        ...state,
        seats: state.seats.map((seat) => (action.payload.includes(seat.id) ? { ...seat, chosen: !seat.chosen } : seat)),
      };
    case types.RESERVE_SEATS:
      return {
        ...state,
        seats: state.seats.map((seat) => (action.payload.includes(seat.id) ? { ...seat, reserved: true } : seat)),
      };
